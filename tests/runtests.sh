...
while read -r f; do
	if [[ -e "ft_$f.c" ]]; then
		if [[ -e "tests/test_ft_$f.c" ]]; then
			gcc -g -Wall -Wextra -Werror ft_str*.c ft_mem*.c "tests/test_ft_$f.c" -o runtests && ./runtests
		else
	...
	if [[ -e "ft_$func.c" ]]; then
		gcc -g -Wall -Wextra -Werror ft_str*.c ft_mem*.c ft_sub*.c ft_atoi.c ft_is*.c ft_to*.c  "tests/test_ft_$func.c" -o runtests && ./runtests debug
	else
